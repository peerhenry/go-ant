...
func loadTextureNRGBA(rgba *image.NRGBA) uint32 {
...
func loadTextureRGBA(rgba *image.RGBA) uint32 {
	var textureId uint32
	gl.GenTextures(1, &textureId)
	gl.BindTexture(gl.TEXTURE_2D, textureId)
	gl.TexImage2D(
...
func LoadImageFileToUniform(filePath string, uniformName string, programHandle uint32, textureUnit int32) {
	...
	case *image.RGBA:
		if rgba, ok := i.(*image.RGBA); ok {
			log.Println("image size:", rgba.Bounds().Dx(), "x", rgba.Bounds().Dy())
			loadTextureRGBA(rgba)
			...
			panic("Could not extract RGBA from image...")
		}
	case *image.NRGBA64:
